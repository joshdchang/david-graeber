<script setup>
  const { data: article, pending: p1 } = await useFetch('/api/articles/3')
...
<template>
  <div class="mb-5" v-if="!p1">

    <Head>
      <Title>{{article.data.title}} - David C. Graeber</Title>
    </Head>

    <div class="relative">

      <!-- featured image -->
      <TheImage class="top-image" :asset="article.data.featured_image"></TheImage>
      <!-- darkening overlay -->
      ...
      <div class="md:absolute bottom-0 left-0 right-0 p-9 sm:p-14 md:p-16 lg:p-20 xl:p-24">
        ...
        <p v-if="article.data.description"
    ...
    <div class="article-container">
      <p class="uppercase text-lg md:text-xl lg:text-2xl mb-10 md:mb-16 lg:mb-20 letter-spacing">
        <PrettyDate :dateStr="article.data.custom_publish_date || article.data.date_created"></PrettyDate>
      </p>
      <DynamicText :content="article.data.content"></DynamicText>
